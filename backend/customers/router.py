...
def list_customers(request, filters: CustomerFilterSchema = Query(...)):
    ...
    queryset = Customer.objects.select_related("created_by").distinct()
...
def create_customer(request, data: CustomerCreateSchema):
    ...
    try:
        ...
        if Customer.objects.filter(phone=phone_e164).exists():
        ...
        if hasattr(request, "current_shop") and request.current_shop:
            from .models import CustomerShopHistory
...
def update_customer(request, customer_id: int, data: CustomerUpdateSchema):
    ...
    try:
        ...
        if "phone" in incoming and incoming["phone"]:
            phone_e164 = normalize_phone(incoming["phone"])
            if (
                Customer.objects.filter(phone=phone_e164)
                .exclude(id=customer_id)
                .exists()
            ):
                return 400, {"error": "Клиент с таким номером телефона уже существует"}
            incoming["phone"] = phone_e164

        # Проверяем email
        if "email" in incoming and incoming["email"]:
            email_norm = incoming["email"].strip()
            if (
                Customer.objects.filter(email__iexact=email_norm)
                .exclude(id=customer_id)
                .exists()
            ):
                return 400, {"error": "Клиент с таким email уже существует"}
            incoming["email"] = email_norm
...
@router.delete(
    "/{customer_id}", response={200: MessageSchema, 403: ErrorSchema, 404: ErrorSchema}
)
...
def get_customer_orders(request, customer_id: int):
    ...
    queryset = Order.objects.filter(
        customer=customer,
        shop__in=request.auth.get_available_shops(),
    )
    current_shop = getattr(request, "current_shop", None)
    if current_shop:
        queryset = queryset.filter(shop=current_shop)
