...
def create_stock_movement(request, data: dict = Body(...)):
    ...
    if current_shop and balance.shop_id != current_shop.id:
        raise PermissionError("Нет доступа к остаткам в другом филиале")
    if not request.auth.can_access_shop(balance.shop):
        raise PermissionError("Нет доступа к остаткам в этом филиале")
...
def receive_purchase_order(request, order_id: int, data: dict = Body(...)):
    ...
    if current_shop and purchase_order.shop_id != current_shop.id:
        raise PermissionError("Нет доступа к заказу поставщика в другом филиале")
    if not request.auth.can_access_shop(purchase_order.shop):
        raise PermissionError("Нет доступа к заказу поставщика в этом филиале")
