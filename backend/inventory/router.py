...
def create_stock_movement(request, data: dict = Body(...)):
    ...
    stock_balance = get_object_or_404(StockBalance, id=data["stock_balance_id"])
    if not request.auth.can_access_shop(stock_balance.shop):
        raise PermissionError("Нет доступа к остаткам этого филиала")
    current_shop = getattr(request, "current_shop", None)
    if current_shop and stock_balance.shop_id != current_shop.id:
        raise PermissionError("Остаток принадлежит другому филиалу")
...
def receive_purchase_order(request, order_id: int, data: dict = Body(...)):
    ...
    purchase_order = get_object_or_404(PurchaseOrder.objects.select_related("shop"), id=order_id)
    if not request.auth.can_access_shop(purchase_order.shop):
        raise PermissionError("Нет доступа к заказу поставщика этого филиала")
    current_shop = getattr(request, "current_shop", None)
    if current_shop and purchase_order.shop_id != current_shop.id:
        raise PermissionError("Заказ поставщика принадлежит другому филиалу")
