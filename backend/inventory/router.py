...
def create_stock_movement(request, data: dict = Body(...)):
    ...
    stock_balance = get_object_or_404(
        StockBalance.objects.select_related("shop"), id=data["stock_balance_id"]
    )
...
def receive_purchase_order(request, order_id: int, data: dict = Body(...)):
    ...
    purchase_order = get_object_or_404(
        PurchaseOrder.objects.select_related("shop"), id=order_id
    )
    if not request.auth.can_access_shop(purchase_order.shop):
        raise PermissionError("Нет доступа к заказу поставщика из другого филиала")
    current_shop = getattr(request, "current_shop", None)
