...
class LoyaltyProgram(models.Model):
    ...
    min_redeem_points = models.PositiveIntegerField(
        "Минимум баллов для списания", default=100
    )
    ...
    shops = models.ManyToManyField(
        Shop, verbose_name="Доступна в магазинах", blank=True
    )
...
class CustomerLoyalty(models.Model):
    ...
    tier_level = models.CharField(
        "Уровень", max_length=20, choices=TierLevel.choices, default=TierLevel.BRONZE
    )
    tier_points_threshold = models.PositiveIntegerField(
        "Баллы для следующего уровня", default=1000
    )

    # Статистика
    total_spent = models.DecimalField(
        "Общая сумма покупок", max_digits=12, decimal_places=2, default=Decimal("0.00")
    )
...
class PointsTransaction(models.Model):
    ...
    customer_loyalty = models.ForeignKey(
        CustomerLoyalty, on_delete=models.CASCADE, related_name="transactions"
    )
    transaction_type = models.CharField(
        "Тип транзакции", max_length=20, choices=TransactionType.choices
    )
    ...
    def __str__(self):
        return (
            f"{self.customer_loyalty.customer.full_name} - "
            f"{self.points} баллов ({self.get_transaction_type_display()})"
        )
...
class LoyaltyReward(models.Model):
    ...
    program = models.ForeignKey(
        LoyaltyProgram, on_delete=models.CASCADE, related_name="rewards"
    )
    name = models.CharField("Название награды", max_length=100)
    reward_type = models.CharField(
        "Тип награды", max_length=20, choices=RewardType.choices
    )
    ...
    required_orders_count = models.PositiveIntegerField("Необходимо заказов", default=0)

    # Параметры награды
    discount_percent = models.DecimalField(
        "Размер скидки (%)", max_digits=5, decimal_places=2, null=True, blank=True
    )
    bonus_points = models.PositiveIntegerField("Бонусные баллы", null=True, blank=True)
...
class CustomerReward(models.Model):
    ...
    customer_loyalty = models.ForeignKey(
        CustomerLoyalty, on_delete=models.CASCADE, related_name="rewards"
    )
