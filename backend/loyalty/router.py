...
@router.get(
    "/customer/{customer_id}/transactions", response=list[PointsTransactionSchema]
)
@paginate
...
def calculate_points_for_order(request, order_id: int):
    ...
    tier_multiplier = (
        customer_loyalty.get_tier_multiplier() if customer_loyalty else 1.0
    )
