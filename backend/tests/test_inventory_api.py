...
class InventoryCrossShopSecurityTests(TestCase):
    """Regression tests for cross-tenant IDOR vulnerabilities in inventory endpoints."""

    def setUp(self):
        self.shop1 = Shop.objects.create(
            name="Shop One", code="SH01", timezone="Europe/Moscow", currency="RUB"
        )
        self.shop2 = Shop.objects.create(
            name="Shop Two", code="SH02", timezone="Europe/Moscow", currency="RUB"
        )
        ...
        # user1 only has access to shop1
        ...
        # Creating the item AFTER both shops exist so signals create balances for both
        ...
        # shop2's balance is auto-created by the post_save signal
        ...
        PurchaseOrderItem.objects.create(
            purchase_order=self.purchase_order_shop2,
            item=self.item,
            ordered_quantity=5,
            unit_price=100,
        )
    ...
    def test_create_stock_movement_blocked_for_other_shop_balance(self):
        """User from shop1 must not be able to create a stock movement for shop2's balance."""
        response = self.client.post(
    ...
    def test_receive_purchase_order_blocked_for_other_shop(self):
        """User from shop1 must not be able to receive a purchase order belonging to shop2."""
        response = self.client.post(
            ...
            data=json.dumps(
                {
                    "items": [
                        {
                            "purchase_order_item_id": self.purchase_order_shop2.items.first().id,
                            "received_quantity": 5,
                        }
                    ]
                }
        ...
        self.assertEqual(response.status_code, 403, response.content)
        # Verify stock was NOT credited to shop2
        self.balance_shop2.refresh_from_db()
        self.assertEqual(self.balance_shop2.quantity, 0)
