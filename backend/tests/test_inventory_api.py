...
class InventoryCrossShopSecurityTests(TestCase):
    ...
    def setUp(self):
        self.shop1 = Shop.objects.create(name="Shop A", code="SHA", currency="RUB")
        self.shop2 = Shop.objects.create(name="Shop B", code="SHB", currency="RUB")

        role = Role.objects.create(name="Warehouse", code="warehouse_idor")
        for codename in (
            "inventory.add_movement",
            "inventory.receive_purchase_orders",
        ):
        ...
        self.balance_shop2, _ = StockBalance.objects.get_or_create(
            shop=self.shop2, item=self.item
        )
        self.balance_shop2.quantity = 10
        self.balance_shop2.save()

        supplier = Supplier.objects.create(name="IDOR Supplier")
    ...
    def test_stock_movement_blocked_for_other_shop_balance(self):
        """User from shop1 must not manipulate stock belonging to shop2."""
        response = self.client.post(
